 ...
 */
#define PTR_SZ			4		/* size of ptr field in hash chain */
#define PTR_MAX			9999	/* max file offset = 10**PTR_SZ - 1 */
#define NHASH_DEF		4		/* default hash table size */
...
DBHANDLE db_open(const char *pathname,int oflag, .../*int mode*/)
{
	...
	/* Allocated a DB structure, and the buffers it needs.
	 */
	len = strlen(pathname);
	...
	if ((oflag & (O_CREAT | O_TRUNC)) == (O_CREAT | O_TRUNC)){
		...
		if(statbuff.st_size == 0){
			 ...
			 * ptrs with a value of 0. The +1 is fro the free 
...
{
	_db_free((DB *)db); /* closes fds, free buffers & struct */
}
...
/*
 * Find the specified record. Called by db_delete, db_fetch, and db_store.
...
static int _db_find_and_lock(DB *db, const char *key, int writelock)
{
	 ...
	 * when done. Note we lock and unlock only the first bytee.
	 */
	if(writelock){
		if(writew_lock(db->idxfd, db->chainoff, SEEK_SET, 1) <0)
			err_dump("_db_find_and_lock: write_lock error");
	}else{
 ...
 * the free list pointer, a hash table chain ptr, or an index 
 * record chain ptr.
 */
...
void test_db_readptr(DB *db, off_t offset)
{
	assert(_db_readptr(db,offset)==53);
}
...
int db_store(DBHANDLE h,const char *key, const char *data, int flag)
{
	...
	if (_db_find_and_lock(db, key, 1) < 0) { /* record not found */
		...
		if (_db_findfree(db, keylen, datlen) < 0) {
				/*
				 * Can't find an empty recor big enough. Append the 
				...
				/*
				 * db->idxoff was set by _db_writeidx. The new record goes to the 
				 * front of the hahs chain.
				 */
